...
def intLorentzian(x, amp, ctr, wid):
    return -amp*wid*np.arctan((ctr - x)/wid);

def areaUnderLorentzian(x0, x1, amp, ctr, wid):
    return intLorentzian(x1, amp, ctr, wid) - intLorentzian(x0, amp, ctr, wid);
...
def fit_lorentzians(guess, func, x, y):
    popt, pcov = curve_fit(func, x, y, p0=guess, maxfev=14000, sigma=None)
    print('popt:', popt)
    fit = func(x, *popt)
    # pyplot.plot(x, y)
    # pyplot.plot(x, fit , 'r-')
